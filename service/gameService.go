...
func (g *GameService) LaunchLevel(clusterName, level string, bin, beta int) {
	...
	if bin == 64 {
		...
		startCmd = "cd " + dstInstallDir + "/bin64 ; screen -d -m -S \"" + screenKey.Key(clusterName, level) + "\" box64 ./dontstarve_dedicated_server_nullrenderer_x64_luajit -console -cluster " + clusterName + " -shard " + level
	} else {
